    ...
    <div className="space-y-4">
      ...
      {warnings.length > 0 && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-amber-800">Warnings</span>
          </div>
      ...
      {projected && (
        <div className="card overflow-hidden">
          <div className="border-b border-border px-4 py-3 pt-5">
              ...
              <tbody>
                {portfolio.project_summaries.map((p, i) => (
                  <tr key={i}>
                    ...
                    <td className="text-right font-mono">{fmt(p.monthly_burn)}</td>
                    <td className={`text-right font-mono ${p.margin_pct > 25 ? "text-emerald-500" : "text-red-500"}`}>{p.margin_pct.toFixed(1)}%</td>
                    <td className={`text-right font-mono ${p.months_remaining < 3 && p.months_remaining > 0 ? "font-semibold text-red-500" : ""}`}>{p.months_remaining.toFixed(1)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t border-border bg-surface-2 font-semibold">
                <tr>
                  <td className="px-5 py-3 text-fg">Portfolio Total</td>
                  <td className="px-5 py-3 text-right font-mono">{fmt(portfolio.total_burn)}</td>
                  <td className="px-5 py-3 text-right font-mono">{portfolio.total_margin_pct.toFixed(1)}%</td>
                  <td className="px-5 py-3 text-right font-mono">{fmt(portfolio.total_margin_dollars)}/mo</td>
                </tr>
              </tfoot>
            </table>
          </div>
...
function CompareRow({ label, before, after, delta, deltaNum, invertColor }: {
  ...
  return (
    <tr>
      <td className="font-medium text-fg">{label}</td>
      <td className="text-right font-mono text-muted">{before}</td>
      <td className="text-right font-mono text-fg">{after}</td>
      <td className={`text-right font-mono font-semibold ${deltaColor(deltaNum, invertColor)}`}>
        {delta}
      </td>
    </tr>
  );
